// Spike G desktop measurement runner.
//
// Drives spike_g.html via headless Chrome (puppeteer-core, system Chrome).
// For each requested benchmark, loads
//   http://127.0.0.1:<port>/?bench=<name>&auto=1
// waits for window.__spikeFResult to be populated (same key as spike-f
// harness), prints a JSON line, and moves to the next bench.
//
// Usage:
//   node run_chrome.cjs --port 8766 [--bench LIST] [--out FILE] [--show]
...
const port      = parseInt(arg("port", "8766"), 10);
  ...
  try {
    for (const bench of benches) {
      ...
      try {
        ...
        try { await page.close(); } catch (e) { console.error("[page.close]", e.message); }
      }
      if (outFile) {
