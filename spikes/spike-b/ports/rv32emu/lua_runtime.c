/* Minimal C runtime for the Lua spike (rv32emu, RV32IMFC ilp32f).
 *
 * Provides everything Lua 5.4 needs from the C library that we are not
 * willing to pull in via a real libc: string, ctype, stdio, stdlib, malloc,
 * setjmp helpers, and a thin transcendental layer.  All output goes to
 * stdout via Linux RV32 ECALL #64 (write).  All "errors" terminate via
 * ECALL #93 (exit) — Lua only longjmps within its own machinery.
 *
 * Goals:
 *  - Correct enough for the benchmarks listed in spikes/spike-b/PLAN.md.
 *  - Small enough to read in one sitting.
 *
 * Non-goals:
 *  - Full ANSI C conformance.
 *  - Locale, signals, files (no FS in the guest).
 *  - Transcendental accuracy beyond sqrt/fabs/floor/ceil/trunc.
 */
...
/*
 * Heap is a fixed 8 MiB region in BSS.  Each block carries an 8-byte header
 * containing its allocated size.  free() pushes onto a singly-linked free
 * list keyed by size; malloc() walks the list looking for the first block
 * large enough.  Adequate for benchmark-scale Lua programs (a few hundred
 * thousand small allocs over the life of one cart run).
 */
...
/* sqrt: emit fsqrt.s / fsqrt.d directly.  rv32imfc has F (single) but no D —
 * so for sqrt(double) we narrow to float, sqrt, and widen back.  Lua only
 * calls sqrt via lmathlib's l_mathop wrappers, which under LUA_32BITS go
 * through sqrtf — so the double form is rarely hit. */
float sqrtf(float x)
{
    float r;
...
/* ldexp/frexp: Lua 5.4 uses these only via math.* — none of our benchmarks
 * call them.  Stubs return their input. */
...
/* ── snprintf / printf ───────────────────────────────────────────────────── */
/*
 * Supports: %d %i %u %x %X %o %p %s %c %%, %f %g %e (double), with
 * flags '-' '+' '0' ' ' '#', width, precision, and length 'l' / 'll' / 'z'.
 * The float code uses simple decimal split and is not bit-perfect with
 * glibc's printf — fine for benchmark output, not for round-tripping.
 */
                     ...
                     int flag_plus, int flag_space)
{
    ...
    /* Reserve a column for the sign by reducing the bare-number width. */
    int signw = (neg || flag_plus || flag_space) ? 1 : 0;
    int eff_width = width - signw;
    if (eff_width < 0) eff_width = 0;

    /* Handle zero-padding manually so the sign goes outside the zeros. */
    ...
    /* Otherwise pad with spaces around the signed number. */
    char num[34];
    int n = 0;
...
/* Strategy: split into integer + fractional parts, round fractional to the
 * requested precision.  No exponent for %f.  %e always emits exponent form.
 * %g picks %e if |x| < 1e-4 or >= 10^prec, otherwise %f, with trailing zero
 * trimming.  Adequate for benchmark output. */
...
static void emit_double_fixed(pctx *c, double v, int prec, int flag_plus, int flag_space)
{
    ...
    /* break apart */
    /* We don't have a 64-bit-int conversion helper that handles huge values
     * cleanly under softfloat; cap at what fits in a uint32 to avoid
     * overflow.  Benchmark output doesn't exceed this range. */
...
{
    if (prec <= 0) prec = 6;
    /* Always use fixed; trim trailing zeros after the decimal point. */
    pctx tmp;
    char tbuf[64];
    ...
    if (dot < len) {
        ...
        if (end == dot + 1) end = dot; /* drop dot too */
        len = end;
    }
    pputn(c, tbuf, len);
}
...
/* ── stdio FILE surface ──────────────────────────────────────────────────── */
/* All stdio FILE-using surfaces that Lua brings in via lauxlib (luaL_loadfile,
 * luaL_dofile, luaL_loadfilex, luaB_print's fwrite path) reduce, in our build,
 * to writes on stdout/stderr.  We give stdout/stderr unique non-NULL pointer
 * values that fwrite/fputs/fprintf can branch on. */
