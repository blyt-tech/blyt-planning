// Spike F main-thread harness.
//
// Adapted from spike-e/web/spike_e.js.  Differences:
//   * Cart dropdown becomes bench dropdown; bench_list.js exposes benchFiles.
//   * Worker loads lua.js (Lua-direct-WASM) instead of rv32emu.js.
//   * Result key is window.__spikeFResult (the chrome runner reads this).
//
// All measurement is in milliseconds (the bench prints microseconds on the
...
$run.addEventListener("click", () => {
  ...
  worker = new Worker("spike_f_worker.js");
  worker.onmessage = (ev) => {
    const m = ev.data;
    if (m.type === "stdout") {
      log(m.line);
      ...
      // See spike_e.js commentary — Emscripten flushes stdout post-task on
      // Chrome 147 so SUMMARY trails this message.  We trust SUMMARY.
...
// ── auto-run mode ───────────────────────────────────────────────────────────
// URL params: ?bench=doom_tick&auto=1
