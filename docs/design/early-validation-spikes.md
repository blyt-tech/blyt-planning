...
| [U](#spike-u--hardware-doubles-end-to-end-ilp32d-and-rv32d-in-rv32emu)<br>- `ilp32d` sysroot + custom `riscv32imafdc` Rust target link with no float-ABI mismatch; `EF_RISCV_FLOAT_ABI_DOUBLE` confirmed<br>- RV32D added to `blyt-tech/rv32emu` (NaN-boxing, Berkeley SoftFloat `f64` kernels); `riscv-arch-test` D suite + existing F suite pass<br>- Lua flipped to `int32+float64` (`BLYT_LUA_I32_F64`); cross-path digests byte-equal rv32 (arm64+amd64) and WASM Lua-direct<br>- save-state absorbs 64-bit FP register file; cross-host round-trip byte-equal<br>- transcendental parity: basic ops byte-equal across all three paths; libm source aligned where needed<br>- merged to blyt main as PR #66 (`rv32d-f64-support`, 2026-06-16); Stage 6 (metal) deferred | done | rv64 silicon (Stage 6, deferred) | — | yes | — |
| [V](#spike-v--language-onboarding-generality-swift-as-probe) — architecture probe (Spike U dependency met — ready to execute): use Embedded Swift, a deliberately `Double`-/stdlib-leaning non-Rust LLVM language, to test whether the language-onboarding seams are language-agnostic. Produces a friction log, not a Swift adoption. Full brief: [`spike-v-swift-carts.md`](spike-v-swift-carts.md) | proposed | — | — | — | — |
| [X](spike-x-rasterizer.md) — host-side 2D rasterizer: unify the `acquire`/`present` framebuffer mechanism across the emulated / host-Lua / native execution models, and prove an integer-only rasterizer hashes bit-identically across all four compile targets (host-x86, host-arm, wasm, RV32-native). Validates the integer-only ("Option A") decision and the unified framebuffer contract before the ~50-entry graphics surface is built. Full brief: [`spike-x-rasterizer.md`](spike-x-rasterizer.md) | proposed | — | — | yes | — |
| [Y](spike-y-lua-per-pixel.md) — Lua per-pixel plotting throughput (#195 Stage 2 / #208 spin-out): measured on a Pi Zero 2 W, emulated **and** native. Emulated floor (`-O2` core) reaches ~1.2 k px/frame with the fastest VM fast path (up from ~266 for the shipped `lk:set`), clearing the low end of the 1 k–10 k bar; **native Lua on the same Pi is ~44–58× faster (~52–65 k px/frame, full-screen / PICO-8-class)** and the VM patches barely matter there. Maps the optimization ladder (plain fn / implicit `set_pixel` / `OP_CALL`-inline / fast `OP_SELF` / index-opcode / 2-axis). **Key: the order-of-magnitude lever is running Lua native (host-Lua fast path), not per-pixel VM patches — which only help the emulated hybrid-on-handheld case.** Full brief: [`spike-y-lua-per-pixel.md`](spike-y-lua-per-pixel.md) | done | Pi Zero 2 W (done) | — | yes (extend host-Lua fast path to the native player; repo ships `-O0` by default) | — |
| [Z](spike-z-native-hostlua-fp-determinism.md) — Native host-Lua FP determinism: the FMA + cross-arch gate before host-Lua-everywhere (Spike Y follow-on; blyt#223 Phase A extended off WASM). Proves whether native host-Lua (x86-64 / arm64) reproduces the SoftFloat reference **bit-for-bit** where WASM structurally can't — the `-ffp-contract=off` Zone-1 claim on hardware FMA (`a*b+c`), the native `blyt_fpm` realization (native-f64 vs `-msoft-float`, incl. x86-64 codegen viability), and the cross-arch host-vs-host digest gate (the netplay proof). Produces the determinism go/no-go for retiring emulated-RV32-Lua on non-RISC-V hosts; does not make that decision. **Result: GO** — native host-Lua == SoftFloat reference bit-for-bit on real arm64 + real x86-64 (CI) + wasm; ship Mode A (`-msoft-float` not viable); Q4 strtod deferred to Phase B. Full brief: [`spike-z-native-hostlua-fp-determinism.md`](spike-z-native-hostlua-fp-determinism.md) · results: [`spike-z-results.md`](spike-z-results.md) | done (GO) | arm64 (real) + x86-64 (real, CI) + wasm | — | **DECISION MADE 2026-07-07: host-Lua-everywhere is GO (ADR-0136).** Both gates green (Y throughput ~50× on the Pi incl. a Doom-shaped workload; Z determinism bit-exact incl. Phase B). Implementation = host-Lua on the native players (epic tracked in blyt). | — |
