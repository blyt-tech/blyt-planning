...
Accepted — amended by Spike E/F findings and ADR-0111:

- **WASM target uses the host-embedded Lua fallback.** Spike E measured
  Lua-in-RV32IMFC-in-WASM at 7.4× over the 16.67 ms frame budget on
  desktop Chrome (catastrophic on mid-range Android). Spike F confirmed
  Lua compiled directly to WASM runs at ~10% of budget on the same
  workload. The WASM target therefore uses the host-embedded Lua
  architecture named as the fallback in this ADR's Context section. The
  primary architecture (Lua as an RV32IMAFC library inside rv32emu)
  remains in use for hardware, Pi, and desktop-native targets.

- **Lua+Rust hybrid calling on the WASM target.** On the WASM target,
  Lua runs in the WASM host while Rust code runs inside rv32emu. The
  `cart_lua_modules` hook is not called on WASM. Instead, the host reads
  a `.lua_exports` ELF section generated by the `#[lua_export]` proc
  macro and registers typed host-side trampolines that marshal arguments
  across the host→guest boundary. See ADR-0111 for the full design.
...
- **WASM performance question — resolved.** Spike E measured
  Lua-in-RV32IMFC-in-WASM at 7.4× over the 16.67 ms frame budget on
  desktop Chrome (Apple Silicon, Chrome 147); the mid-range Android
  projection was 15–30× over. Spike F measured Lua compiled directly to
  WASM (no rv32emu layer) at 1.71 ms mean / 3.40 ms p99 on the same
  workload — ~10%/20% of budget. The WASM target therefore uses the
  host-embedded Lua fallback; the primary architecture applies on all
  other targets. The asymmetric security and debugging model noted in this
  ADR's Context section remains a follow-up work item for the WASM target.
