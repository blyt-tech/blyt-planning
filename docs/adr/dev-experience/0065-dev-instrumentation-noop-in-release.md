# ADR-0065: Dev instrumentation — fc_dev_* as no-ops in release builds
...
**All dev instrumentation functions are prefixed `fc_dev_*` and compile to
...
void fc_dev_watch_f32(const char *name, float value);
void fc_dev_watch_i32(const char *name, int32_t value);

// Named timing sections
void fc_dev_start(const char *section_name);  // begin timing
void fc_dev_finish(const char *section_name); // end timing, record to overlay

// Log a message to the dev console (not visible in release)
void fc_dev_log(const char *fmt, ...);
```
...
defines `FC_RELEASE` when the packer invokes the compiler for a release build.

In release builds (`FC_RELEASE` defined), these functions are either:
...
**RGBA vs palette:** `fc_dev_rect` uses RGBA because dev overlays need to be
...
- `fc_dev_start`/`fc_dev_finish` (not `fc_dev_end`) is chosen to distinguish
