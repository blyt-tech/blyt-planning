...
**CLI packer:** `blyt pack ./myproject` produces `myproject.blyt`. The
...
`blyt run myproject.blyt` for local testing; `blyt run ./myproject`
...
their file managers and shells. `blyt new` writes a project-level
`.gitignore` that excludes it (see below).

**Watch mode:** `blyt watch ./myproject` runs the incremental build loop:
...
protocol; `blyt watch` signals hot reload via this same connection using
...
- `blyt new` turns "start from blank files" into "start from a working
