...
without declaration:** `FC_VG_MUSIC`, `FC_VG_SFX`, `FC_VG_VOICE`. These
correspond to the three categories most carts need, and the runtime
automatically provides matching volume preference handles (`FC_PREF_MUSIC_VOLUME`,
`FC_PREF_SFX_VOLUME`, `FC_PREF_VOICE_VOLUME`) for use in options menus.
...
`fc_voice_play()` time and persist for the voice's lifetime. Groups support:

- Per-group volume control (scales with the three-layer volume model, ADR-0055).
- Per-group mute/unmute.
- `fc_voice_stop_group(FC_VG_SFX)` — stop all voices in a group.
...
```c
fc_voice_play(R_FOOTSTEP, VG_GAMEPLAY, MY_TAG_FOOTSTEPS, &voice);
```

`fc_voice_stop_tag(MY_TAG_FOOTSTEPS)` stops all currently active voices
