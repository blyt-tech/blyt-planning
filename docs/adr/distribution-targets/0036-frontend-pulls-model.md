...
library (`libfantasyconsole`); it has no event loop of its own. Frontends
...
// or the rate declared by fc_cart_fps()).
fc_result_t fc_runtime_update(fc_runtime_h rt);

// Render the current frame into the runtime-owned framebuffer.
fc_result_t fc_runtime_draw(fc_runtime_h rt);
```

The frontend is responsible for:
1. Measuring wall-clock elapsed time each frame.
2. Accumulating it and calling `fc_runtime_update()` once per logical tick,
   up to a maximum of 3 catch-up ticks per render to avoid spiral-of-death.
3. Calling `fc_runtime_draw()` once per render frame.
4. Reading the framebuffer via `fc_runtime_get_framebuffer()` and blitting
   it to the display.

The cart declares its preferred tick rate via `fc_cart_fps()` (read from
