...
The packer generates constants of type `fc_field_h` (an integer typedef):

```c
// Generated: cart_state.h
#define S_PLAYERS  ((fc_buffer_h)1)
#define S_ENEMIES  ((fc_buffer_h)2)

// Field handles (fc_field_h = uint32_t)
#define S_PLAYER_X      ((fc_field_h)0x00010001)
#define S_PLAYER_Y      ((fc_field_h)0x00010002)
#define S_ENEMY_HP      ((fc_field_h)0x00020001)
// ...
```
...
```c
fc_buffer_set_f32(S_PLAYERS, slot, S_PLAYER_X, x);
float hp = fc_buffer_get_f32(S_ENEMIES, slot, S_ENEMY_HP);
```

In Lua, the same constants are integers; the SOA metatable sugar
(ADR-0011) wraps them for ergonomic access.

`FC_FIELD_NONE` (zero) is the sentinel for an invalid or missing field.
